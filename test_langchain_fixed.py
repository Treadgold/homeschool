#!/usr/bin/env python3
"""
Fixed standalone test script for the LangChain agent service.
This uses proper escaping for JSON format in prompts.
"""
...
async def test_fixed_langchain():
    """Test the LangChain agent with properly escaped prompts."""
    
    print("🧪 Testing LangChain Agent (Fixed)...")
    print("=" * 50)
    ...
    try:
        ...
        # Use string concatenation to avoid template variable conflicts
        system_message = f"""You are a helpful assistant that can use tools to help users.
{tools_definition}
...
If no tool is needed, just respond normally."""
    ...
    try:
        reasoning_system = f"""You are a helpful assistant. Your job is to decide if you should use a tool to answer the user's question.
{tools_definition}
...
if __name__ == "__main__":
    asyncio.run(test_fixed_langchain()) 